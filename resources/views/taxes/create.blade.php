<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-gray-800">إضافة ضريبة جديدة</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select name="amount_type" id="amount_type" required
                        class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        <option value="">اختر النوع</option>
                        <option value="fixed" {{ old('amount_type') === 'fixed' ? 'selected' : '' }}>ثابت</option>
                        <option value="percent" {{ old('amount_type') === 'percent' ? 'selected' : '' }}>نسبة مئوية</option>
                        <option value="group" {{ old('amount_type') === 'group' ? 'selected' : '' }}>مجموعة</option>
                        <option value="division" {{ old('amount_type') === 'division' ? 'selected' : '' }}>قسمة</option>
                    </select>
                </div>

                <div>
                    <label for="rate" class="mb-1 block text-sm font-medium text-gray-700">النسبة / المعدل <span class="text-red-500">*</span></label>
                    <input type="number" name="rate" id="rate" value="{{ old('rate', 0) }}" step="0.01" min="0" required
                        class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>
                ...
                <div class="flex items-end gap-6">
                    ...
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_included_in_price" value="1" {{ old('is_included_in_price') ? 'checked' : '' }}
                            class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
