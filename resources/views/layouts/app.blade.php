        ...
        <aside :class="sidebarOpen ? 'w-64' : 'w-20'"
            ...
            <nav class="flex-1 overflow-y-auto sidebar-scroll py-4">
                <ul class="space-y-1 px-3">
                    ...
                    <!-- Treasury & Banks -->
                    <li x-data="{ open: {{ request()->routeIs('cash-treasuries.*') || request()->routeIs('bank-accounts.*') || request()->routeIs('payments.*') || request()->routeIs('transfers.*') ? 'true' : 'false' }} }">
                        <button @click="open = !open"
                            class="menu-item w-full flex items-center justify-between gap-3 px-4 py-3 rounded-xl {{ request()->routeIs('cash-treasuries.*') || request()->routeIs('bank-accounts.*') || request()->routeIs('payments.*') || request()->routeIs('transfers.*') ? 'active' : 'hover:bg-primary-700' }} transition-all">
                            <div class="flex items-center gap-3">
                                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <ul x-show="open" x-collapse class="mt-1 mr-4 space-y-1">
                            <li>
                                <a href="{{ route('cash-treasuries.index') }}"
                                    class="submenu-item block px-4 py-2 rounded-lg text-sm {{ request()->routeIs('cash-treasuries.*') && !request()->routeIs('payments.*') && !request()->routeIs('transfers.*') ? 'active' : 'hover:bg-primary-700' }}">
                                    الخزينة النقدية
                                </a>
                            </li>
                            <li>
