<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('items.index', ['all' => 1]) }}" target="_blank" class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 transition"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg> طباعة</a>
            ...
            @else
            <h2 class="text-xl font-bold text-gray-800">طباعة الأصناف</h2>
    ...
    <div class="rounded-xl bg-white shadow-sm border border-gray-200 p-6">
        ...
        <div class="mb-4 flex flex-wrap items-center gap-2">
            ...
            @php $lastSkuPerCategory = $lastSkuPerCategory ?? collect(); @endphp
            ...
            <table class="w-full text-right text-sm">
                <thead>
                    <tr class="border-b border-gray-200 bg-gray-50">
                        <th class="px-1 py-1 font-semibold text-gray-700 w-[50px] no-print">صورة</th>
                        <th class="px-1 py-1 font-semibold text-gray-700 w-[50px]">الكود</th>
                        <th class="px-1 py-1 font-semibold text-gray-700 w-auto min-w-[160px]">اسم الصنف</th>
                        <th class="px-1 py-1 font-semibold text-gray-700 w-[70px]">التصنيف</th>
                        <th class="px-1 py-1 font-semibold text-gray-700 w-[45px]">الوحدة</th>
                        <th class="px-1 py-1 font-semibold text-gray-700 text-left w-[90px]">سعر الشراء</th>
                        <th class="px-1 py-1 font-semibold text-gray-700 text-left w-[90px]">سعر البيع</th>
                        <th class="px-1 py-1 font-semibold text-gray-700 text-left w-[55px]">المخزون</th>
                        <th class="px-1 py-1 font-semibold text-gray-700 text-center w-[60px] no-print">إجراءات</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($items as $item)
                        <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                            <td class="px-1 py-0.5 no-print">
                                @if($item->image)
                                    <img src="{{ asset($item->image) }}" class="h-7 w-7 rounded border object-cover">
                                @else
                                    <div class="h-7 w-7 rounded border border-dashed border-gray-300 flex items-center justify-center text-gray-400">
                            ...
                            <td class="px-1 py-0.5 font-mono text-[10px] font-bold text-gray-600 w-[50px]">{{ $item->sku ?? '#' . $item->id }}</td>
                            <td class="px-1 py-0.5 w-auto min-w-[160px]">
                                <a href="{{ route('items.show', $item) }}" class="font-medium text-gray-900 hover:text-blue-600 text-[13px]">{{ $item->name }}</a>
                                @if($item->barcode)
                                    <div class="text-[9px] text-gray-500">باركود: {{ $item->barcode }}</div>
                                @endif
                            </td>
                            <td class="px-1 py-0.5 text-gray-600 w-[70px] text-[11px]">{{ $item->category->name ?? '-' }}</td>
                            <td class="px-1 py-0.5 text-gray-600 w-[45px] text-[11px]">{{ $item->unit->name ?? '-' }}</td>
                            <td class="px-1 py-0.5 text-left font-mono text-[11px] w-[90px]">{{ number_format($item->cost_price, 2) }} <span class="text-[9px] text-gray-500">{{ $item->purchaseCurrency->symbol ?? $item->purchaseCurrency->code ?? '' }}</span></td>
                            <td class="px-1 py-0.5 text-left font-mono text-[11px] text-emerald-600 w-[90px]">{{ number_format($item->selling_price, 2) }} <span class="text-[9px] text-gray-500">{{ $item->salesCurrency->symbol ?? $item->salesCurrency->code ?? '' }}</span></td>
                            <td class="px-1 py-0.5 text-left w-[55px]">
                                @php $totalStock = $item->warehouses->sum('quantity'); @endphp
                                <span class="inline-flex items-center rounded-full px-1 py-0.5 text-[9px] font-medium {{ $totalStock <= 0 ? 'bg-red-100 text-red-800' : ($totalStock <= ($item->minimum_stock ?? 0) ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800') }}">
                                    {{ $totalStock }}
                                </span>
                                @if(($item->opening_stock ?? 0) > 0)
                                    <div class="text-[9px] text-gray-400 mt-0">افتتاحي: {{ number_format($item->opening_stock, 0) }}</div>
                                @endif
                            </td>
                            <td class="px-1 py-0.5 text-center w-[60px] no-print">
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-2 py-8 text-center text-gray-500">لا توجد أصناف</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
