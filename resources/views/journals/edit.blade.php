<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-gray-800">تعديل الدفتر: {{ $journal->name }}</h2>
            <a href="{{ route('journals.index') }}" class="inline-flex items-center gap-2 rounded-lg bg-gray-600 px-4 py-2 text-sm font-medium text-white hover:bg-gray-700 transition">إلغاء</a>
        </div>
    </x-slot>
    ...
    <div class="rounded-xl bg-white shadow-sm border border-gray-200 p-6">
        <form action="{{ route('journals.update', $journal) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="code" class="mb-1 block text-sm font-medium text-gray-700">كود الدفتر <span class="text-red-500">*</span></label>
                    <input type="text" name="code" id="code" value="{{ old('code', $journal->code) }}" required maxlength="10"
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $journal->name) }}" required
                        class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>
                ...
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        <option value="sale" {{ old('type', $journal->type) === 'sale' ? 'selected' : '' }}>مبيعات</option>
                        <option value="purchase" {{ old('type', $journal->type) === 'purchase' ? 'selected' : '' }}>مشتريات</option>
                        <option value="cash" {{ old('type', $journal->type) === 'cash' ? 'selected' : '' }}>نقدية</option>
                        <option value="bank" {{ old('type', $journal->type) === 'bank' ? 'selected' : '' }}>بنكية</option>
                        <option value="general" {{ old('type', $journal->type) === 'general' ? 'selected' : '' }}>عامة</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="default_account_id" id="default_account_id"
                        ...
                        @foreach($accounts as $account)
                            <option value="{{ $account->id }}" {{ old('default_account_id', $journal->default_account_id) == $account->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="currency_id" id="currency_id"
                        ...
                        @foreach($currencies as $currency)
                            <option value="{{ $currency->id }}" {{ old('currency_id', $journal->currency_id) == $currency->id ? 'selected' : '' }}>
