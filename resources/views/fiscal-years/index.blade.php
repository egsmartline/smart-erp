<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-gray-800">السنوات المالية</h2>
            <a href="{{ route('fiscal-years.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 transition">
                ...
                <tbody>
                    @forelse($fiscalYears as $year)
                        <tr class="border-b border-gray-100 hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">{{ \Carbon\Carbon::parse($year->start_date)->format('Y/m/d') }}</td>
                            <td class="px-4 py-3">{{ \Carbon\Carbon::parse($year->end_date)->format('Y/m/d') }}</td>
                            <td class="px-4 py-3 text-center">
                                @if($year->is_current)
                                    <span class="inline-flex items-center rounded-full bg-emerald-100 px-2.5 py-0.5 text-xs font-medium text-emerald-800">الحالية</span>
                                @else
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="inline-flex items-center gap-1">
                                    ...
                                    <form action="{{ route('fiscal-years.destroy', $year) }}" method="POST" class="inline" onsubmit="return confirm('هل أنت متأكد؟')">
                                        @csrf
                                        @method('DELETE')
