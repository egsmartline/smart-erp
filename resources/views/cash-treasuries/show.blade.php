<x-app-layout>
    ...
    <div class="rounded-xl bg-white shadow-sm border border-gray-200 mt-6">
        <div class="border-b border-gray-200 px-6 py-4">
            <h3 class="text-lg font-bold text-gray-800">تحويل بين الخزينة والحسابات البنكية</h3>
        </div>
        <div class="grid gap-6 p-6 lg:grid-cols-2">
            <div class="rounded-lg border border-gray-200 bg-gray-50 p-4">
                <h4 class="mb-3 font-semibold text-gray-800">تحويل من هذه الخزينة إلى حساب بنكي</h4>
                @if($bankAccounts->isEmpty())
                    <p class="text-sm text-gray-500">لا توجد حسابات بنكية متاحة حاليًا.</p>
                @else
                    <form method="POST" action="{{ route('transfers.store') }}" class="space-y-3">
                        @csrf
                        <input type="hidden" name="from_type" value="treasury">
                        <input type="hidden" name="from_id" value="{{ $treasury->id }}">
                        <input type="hidden" name="to_type" value="bank">
                        <input type="hidden" name="redirect_to" value="{{ route('cash-treasuries.show', $treasury) }}">
                        <div>
                            <label class="mb-1 block text-sm font-medium text-gray-700">الحساب البنكي</label>
                            <select name="to_id" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500" required>
                                <option value="">اختر الحساب البنكي</option>
                                @foreach($bankAccounts as $bank)
                                    <option value="{{ $bank->id }}" {{ old('to_id') == $bank->id ? 'selected' : '' }}>{{ $bank->account_name }} - {{ $bank->bank_name }} ({{ number_format($bank->current_balance, 2) }})</option>
                                @endforeach
                            </select>
                        </div>
            ...
            <div class="rounded-lg border border-gray-200 bg-gray-50 p-4">
                <h4 class="mb-3 font-semibold text-gray-800">تحويل من حساب بنكي إلى هذه الخزينة</h4>
                @if($bankAccounts->isEmpty())
                    <p class="text-sm text-gray-500">لا توجد حسابات بنكية متاحة حاليًا.</p>
                @else
                    <form method="POST" action="{{ route('transfers.store') }}" class="space-y-3">
                        @csrf
                        <input type="hidden" name="from_type" value="bank">
                        <input type="hidden" name="to_type" value="treasury">
                        <input type="hidden" name="to_id" value="{{ $treasury->id }}">
                        <input type="hidden" name="redirect_to" value="{{ route('cash-treasuries.show', $treasury) }}">
                        <div>
                            <label class="mb-1 block text-sm font-medium text-gray-700">الحساب البنكي</label>
                            <select name="from_id" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500" required>
                                <option value="">اختر الحساب البنكي</option>
                                @foreach($bankAccounts as $bank)
                                    <option value="{{ $bank->id }}" {{ old('from_id') == $bank->id ? 'selected' : '' }}>{{ $bank->account_name }} - {{ $bank->bank_name }} ({{ number_format($bank->current_balance, 2) }})</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="grid gap-3 md:grid-cols-2">
                        ...
                        <button type="submit" class="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-700 transition">
                            تنفيذ التحويل إلى الخزينة
                        </button>
                    </form>
                @endif
            </div>
