<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-gray-800">إنشاء إذن استلام</h2>
            <a href="{{ route('purchase-receipt-notes.index') }}" class="inline-flex items-center gap-2 rounded-lg bg-gray-600 px-4 py-2 text-sm font-medium text-white hover:bg-gray-700 transition">
    ...
    <div class="rounded-xl bg-white shadow-sm border border-gray-200 p-6">
        <form action="{{ route('purchase-receipt-notes.store') }}" method="POST" id="receiptForm">
            @csrf
            <input type="hidden" name="lines" id="linesInput">

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    <label for="purchase_order_id" class="mb-1 block text-sm font-medium text-gray-700">أمر الشراء <span class="text-red-500">*</span></label>
            ...
            <div class="border border-gray-200 rounded-lg overflow-hidden">
                <div class="bg-gray-50 px-4 py-3 border-b border-gray-200">
                    <h3 class="text-sm font-bold text-gray-700">أصناف أمر الشراء</h3>
                </div>
                <div id="orderInfo" class="px-4 py-3 text-sm text-gray-500">
                    الرجاء اختيار أمر شراء لعرض الأصناف
                </div>
                    ...
                    <table class="w-full text-right text-sm" id="linesTable" style="display:none;">
                        <thead>
                            <tr class="border-b border-gray-200 bg-gray-50">
                                ...
                                <th class="px-4 py-2 font-semibold text-gray-600">الكمية المستلمة</th>
                                <th class="px-4 py-2 font-semibold text-gray-600">الكمية للاستلام</th>
            ...
            <div class="flex items-center gap-3 border-t border-gray-200 pt-6 mt-6">
                ...
                <a href="{{ route('purchase-receipt-notes.index') }}" class="rounded-lg bg-gray-200 px-6 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-300 transition">
                    إلغاء
                </a>
            </div>
        </form>
    ...
    <script>
        document.getElementById('purchase_order_id').addEventListener('change', function() {
            ...
            if (!orderId) {
                orderInfo.textContent = 'الرجاء اختيار أمر شراء لعرض الأصناف';
                orderInfo.style.display = 'block';
                linesTable.style.display = 'none';
                return;
            }

            fetch('/api/purchase-orders/' + orderId + '/lines')
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    orderInfo.innerHTML = '<strong>المورد:</strong> ' + (data.supplier?.name || '') +
                    ...
                    if (!data.lines || data.lines.length === 0) {
                        linesBody.innerHTML = '<tr><td colspan="6" class="px-4 py-4 text-center text-gray-500">لا توجد أصناف في أمر الشراء</td></tr>';
                        return;
                    }

                    data.lines.forEach(function(line) {
                        var remaining = line.quantity - (line.received_qty || 0);
                        ...
                        tr.innerHTML =
                            ...
                            '<td class="px-4 py-2">' + (line.received_qty || 0) + '</td>' +
        ...
        document.getElementById('receiptForm').addEventListener('submit', function(e) {
            ...
            rows.forEach(function(row) {
                ...
                lines.push({
                    purchase_order_line_id: parseInt(qtyInput.dataset.lineId),
            ...
            if (lines.length === 0) {
                alert('الرجاء إدخال كمية صالحة للاستلام على الأقل');
                e.preventDefault();
                return;
            }
