<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-gray-800">إذن استلام - {{ $purchaseReceiptNote->receipt_number }}</h2>
            <a href="{{ route('purchase-receipt-notes.index') }}" class="inline-flex items-center gap-2 rounded-lg bg-gray-600 px-4 py-2 text-sm font-medium text-white hover:bg-gray-700 transition">العودة للقائمة</a>
        </div>
    </x-slot>
    ...
    <div class="rounded-xl bg-white shadow-sm border border-gray-200 p-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            <div>
                <label class="mb-1 block text-sm font-medium text-gray-500">أمر الشراء</label>
                <p class="text-gray-900 text-sm">{{ $purchaseReceiptNote->purchaseOrder->order_number ?? '-' }}</p>
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-gray-500">المورد</label>
                <p class="text-gray-900 text-sm">{{ $purchaseReceiptNote->supplier->name ?? '-' }}</p>
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-gray-500">المخزن</label>
                <p class="text-gray-900 text-sm">{{ $purchaseReceiptNote->warehouse->name ?? '-' }}</p>
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-gray-500">بواسطة</label>
                <p class="text-gray-900 text-sm">{{ $purchaseReceiptNote->user->name ?? '-' }}</p>
            </div>
            <div class="md:col-span-2">
                <label class="mb-1 block text-sm font-medium text-gray-500">ملاحظات</label>
                <p class="text-gray-900 text-sm">{{ $purchaseReceiptNote->notes ?? '-' }}</p>
            </div>
        </div>
        ...
        <div class="border-t border-gray-200 pt-6">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-right text-sm">
                    ...
                    <tbody>
                        @foreach($purchaseReceiptNote->lines as $index => $line)
                    ...
                    <tfoot>
                        <tr class="border-t-2 border-gray-300 bg-gray-50 font-bold">
                            ...
                            <td class="px-4 py-3">{{ number_format($purchaseReceiptNote->lines->sum('total'), 2) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
