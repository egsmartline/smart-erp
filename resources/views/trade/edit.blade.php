                    ...
                    <select name="party_id" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        ...
                        @if($tradeOperation->type === 'export')
                            @foreach($customers as $c)
                                <option value="{{ $c->id }}" data-type="customer" {{ old('party_id', $tradeOperation->party_id) == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                            @endforeach
                        @else
                            @foreach($suppliers as $s)
                                <option value="{{ $s->id }}" data-type="supplier" {{ old('party_id', $tradeOperation->party_id) == $s->id ? 'selected' : '' }}>{{ $s->name }}</option>
                            @endforeach
                        @endif
                    </select>
                    <input type="hidden" name="party_type" value="{{ $tradeOperation->type === 'export' ? 'customer' : 'supplier' }}">
                </div>
                <div>
