<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-gray-800">تقرير المبيعات</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div class="rounded-xl bg-blue-50 border border-blue-200 p-6 text-center">
            <div class="text-sm text-blue-600 mb-2">إجمالي المبيعات</div>
            <div class="text-2xl font-bold text-blue-700">{{ number_format($totalSales, 2) }} ج.م</div>
        </div>
                ...
                <tbody>
                    @forelse($invoices as $invoice)
                        <tr class="border-b border-gray-100 hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 text-left font-mono">{{ number_format($invoice->subtotal, 2) }}</td>
                            <td class="px-4 py-2 text-left font-mono">{{ number_format($invoice->tax_amount, 2) }}</td>
