            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select name="payment_method" id="payment_method" required class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        ...
                        <option value="cash" {{ old('payment_method') === 'cash' ? 'selected' : '' }}>نقاط</option>
                ...
                <div>
                    <label for="currency_code" class="mb-1 block text-sm font-medium text-gray-700">العملة <span class="text-red-500">*</span></label>
                    <select name="currency_code" id="currency_code" required class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        <option value="">اختر العملة</option>
                        <option value="EGP" {{ old('currency_code') === 'EGP' ? 'selected' : '' }}>جنيه مصري (EGP)</option>
                ...
                <div id="bank_account_field">
                    <label for="bank_account_id" class="mb-1 block text-sm font-medium text-gray-700">الحساب البنكي</label>
                ...
                <div id="cheque_number_field">
                    <label for="cheque_number" class="mb-1 block text-sm font-medium text-gray-700">رقم الشيك</label>
                    <input type="text" name="cheque_number" id="cheque_number" value="{{ old('cheque_number') }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>

                <div>
                    <label for="reference_number" class="mb-1 block text-sm font-medium text-gray-700">رقم المرجع</label>
                    <input type="text" name="reference_number" id="reference_number" value="{{ old('reference_number') }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function toggleFields() {
                ...
                customerField.style.display = type === 'receipt' ? 'block' : 'none';
                supplierField.style.display = type === 'payment' ? 'block' : 'none';
                bankAccountField.style.display = method === 'bank_transfer' ? 'block' : 'none';
                chequeField.style.display = method === 'check' ? 'block' : 'none';
            }
