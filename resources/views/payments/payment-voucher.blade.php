...
<head>
    <meta charset="UTF-8">
    <title>إذن صرف - {{ $payment->payment_number }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="receipt">
        <div class="header">
            <h2>إذن صرف {{ $payment->payment_method === 'cash' ? 'نقدية' : 'شيكات' }}</h2>
            <div class="info">
                <div>رقم الإذن: <span>{{ $payment->payment_number }}</span></div>
        ...
        <div>
            <div class="field">
                <span class="field-label">صرفنا للسيد:</span>
                <span class="field-value">{{ $payment->supplier?->name ?? $payment->customer?->name ?? $payment->account?->name ?? '______________________' }}</span>
            </div>
