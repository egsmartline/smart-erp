<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-gray-800">تعديل الدفعة: {{ $payment->payment_number }}</h2>
        ...
        <form action="{{ route('payments.update', $payment) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="date" id="date" value="{{ old('date', $payment->date->format('Y-m-d')) }}" required class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" name="amount" id="amount" value="{{ old('amount', $payment->amount) }}" step="0.01" min="0" required class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>
                <div>
                    ...
                    <select name="currency_id" id="currency_id" required class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        @foreach($currencies as $currency)
                            <option value="{{ $currency->id }}" {{ old('currency_id', $payment->currency_id) == $currency->id ? 'selected' : '' }}>{{ $currency->name }} ({{ $currency->code }})</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label for="exchange_rate" class="mb-1 block text-sm font-medium text-gray-700">سعر الصرف</label>
                    <input type="number" name="exchange_rate" id="exchange_rate" value="{{ old('exchange_rate', $payment->exchange_rate) }}" step="0.000001" min="0" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>
                ...
                <div id="treasury_field">
                    ...
                    <select name="treasury_id" id="treasury_id" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        ...
                        @foreach($treasuries as $treasury)
                            <option value="{{ $treasury->id }}" {{ old('treasury_id', $payment->treasury_id) == $treasury->id ? 'selected' : ($loop->first && !old('treasury_id') && !$payment->treasury_id ? 'selected' : '') }}>{{ $treasury->name }} ({{ $treasury->code }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div id="bank_account_field">
                    ...
                    <select name="bank_account_id" id="bank_account_id" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        ...
                        @foreach($bankAccounts as $account)
                            <option value="{{ $account->id }}" {{ old('bank_account_id', $payment->bank_account_id) == $account->id ? 'selected' : '' }}>{{ $account->bank_name }} - {{ $account->account_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div id="cheque_number_field">
                    ...
                    <input type="text" name="check_number" id="check_number" value="{{ old('check_number', $payment->check_number) }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>

                <div>
                    <label for="reference" class="mb-1 block text-sm font-medium text-gray-700">رقم المرجع</label>
                    <input type="text" name="reference" id="reference" value="{{ old('reference', $payment->reference) }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>
                ...
                <div class="md:col-span-2 lg:col-span-3">
                    ...
                    <textarea name="notes" id="notes" rows="2" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="وصف العملية">{{ old('notes', $payment->notes) }}</textarea>
                </div>
            </div>
