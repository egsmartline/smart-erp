<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('customers.index') }}" class="inline-flex items-center gap-2 rounded-lg bg-gray-600 px-4 py-2 text-sm font-medium text-white hover:bg-gray-700 transition">
    ...
    <div class="rounded-xl bg-white shadow-sm border border-gray-200 p-6">
        <form action="{{ route('customers.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div>
                    <label for="name" class="mb-1 block text-sm font-medium text-gray-700">اسم العميل <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label for="classification" class="mb-1 block text-sm font-medium text-gray-700">التصنيف</label>
                    <select name="classification" id="classification"
                        class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        <option value="">اختر التصنيف</option>
                        <option value="gold" {{ old('classification') === 'gold' ? 'selected' : '' }}>ذهبي</option>
                        <option value="silver" {{ old('classification') === 'silver' ? 'selected' : '' }}>فضي</option>
                        <option value="bronze" {{ old('classification') === 'bronze' ? 'selected' : '' }}>برونزي</option>
                        <option value="regular" {{ old('classification') === 'regular' ? 'selected' : '' }}>عادي</option>
                    </select>
                </div>

                <div>
                    <label for="opening_balance" class="mb-1 block text-sm font-medium text-gray-700">الرصيد الافتتاحي</label>
                    <input type="number" name="opening_balance" id="opening_balance" value="{{ old('opening_balance', 0) }}" step="0.01" min="0"
                        class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>
                ...
                <div>
                    ...
                    <select name="opening_balance_type" id="opening_balance_type"
                        class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        <option value="debit" {{ old('opening_balance_type', 'debit') === 'debit' ? 'selected' : '' }}>مدين (عليه)</option>
                        <option value="credit" {{ old('opening_balance_type') === 'credit' ? 'selected' : '' }}>دائن (له)</option>
                    </select>
                </div>
            ...
            <div class="flex items-center gap-3 border-t border-gray-200 pt-6">
                ...
                <a href="{{ route('customers.index') }}" class="rounded-lg bg-gray-200 px-6 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-300 transition">إلغاء</a>
            </div>
        </form>
    </div>
</x-app-layout>
