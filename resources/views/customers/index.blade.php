    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('customers.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 transition">
    ...
    <div class="rounded-xl bg-white shadow-sm border border-gray-200 p-6">
        <form method="GET" class="mb-6 flex flex-wrap items-end gap-4">
            ...
            <div class="min-w-[180px]">
                ...
                <select name="classification" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                    <option value="">الكل</option>
                    <option value="gold" {{ request('classification') === 'gold' ? 'selected' : '' }}>ذهبي</option>
                    <option value="silver" {{ request('classification') === 'silver' ? 'selected' : '' }}>فضي</option>
                    <option value="bronze" {{ request('classification') === 'bronze' ? 'selected' : '' }}>برونزي</option>
            ...
            <a href="{{ route('customers.index') }}" class="rounded-lg bg-gray-200 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-300 transition">إعادة تعيين</a>
        </form>

        <div class="mb-4 flex items-center gap-4">
            <div class="rounded-lg bg-blue-50 border border-blue-200 px-4 py-2">
                <span class="text-sm text-blue-700">إجمالي أرصدة العملاء: </span>
                <span class="text-sm font-bold text-blue-800">{{ number_format($totalBalance, 2) }} ج.م</span>
            </div>
            <div class="rounded-lg bg-emerald-50 border border-emerald-200 px-4 py-2">
                <span class="text-sm text-emerald-700">عدد العملاء: </span>
                <span class="text-sm font-bold text-emerald-800">{{ $customers->total() }}</span>
            </div>
        </div>
                ...
                <tbody>
                    @forelse($customers as $customer)
                        <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                            <td class="px-4 py-3 font-mono text-xs font-bold text-gray-600">#{{ $customer->id }}</td>
                            <td class="px-4 py-3">
                                <a href="{{ route('customers.show', $customer) }}" class="font-medium text-gray-900 hover:text-blue-600">{{ $customer->name }}</a>
                                @if($customer->name_ar)
                                    <div class="text-xs text-gray-500">{{ $customer->name_ar }}</div>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3">
                                @if($customer->classification)
                                    <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $customer->classification === 'gold' ? 'bg-yellow-100 text-yellow-800' : ($customer->classification === 'silver' ? 'bg-gray-100 text-gray-800' : ($customer->classification === 'bronze' ? 'bg-orange-100 text-orange-800' : 'bg-blue-100 text-blue-800')) }}">
                            ...
                            <td class="px-4 py-3 text-left font-mono text-sm {{ $customer->balance > 0 ? 'text-red-600' : 'text-emerald-600' }}">
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="inline-flex items-center gap-1">
                                    <a href="{{ route('customers.edit', $customer) }}" class="rounded p-1 text-gray-500 hover:bg-blue-50 hover:text-blue-600 transition" title="تعديل">
