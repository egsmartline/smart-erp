<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-gray-800">الحسابات البنكية</h2>
            <a href="{{ route('bank-accounts.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 transition">
            ...
            <table class="w-full text-right text-sm">
                <thead>
                    <tr class="border-b border-gray-200 bg-gray-50">
                        ...
                        <th class="px-4 py-3 font-semibold text-gray-700">IBAN</th>
                ...
                <tbody>
                    @forelse($bankAccounts as $account)
                        <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3 font-mono text-xs text-gray-600">{{ $account->iban ?? '-' }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ $account->currency->name ?? '-' }}</td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="inline-flex items-center gap-1">
                                    <a href="{{ route('bank-accounts.edit', $account) }}" class="rounded p-1 text-gray-500 hover:bg-blue-50 hover:text-blue-600 transition" title="تعديل">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <form action="{{ route('bank-accounts.destroy', $account) }}" method="POST" class="inline" onsubmit="return confirm('هل أنت متأكد؟')">
                                        @csrf
                                        @method('DELETE')
