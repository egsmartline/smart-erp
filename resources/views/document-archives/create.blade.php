<x-app-layout>
    ...
    <div class="rounded-xl bg-white shadow-sm border border-gray-200 p-6 max-w-2xl">
        <form method="POST" action="{{ route('document-archives.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-gray-700">الملف <span class="text-red-500">*</span></label>
                    <input type="file" name="file" required
                        class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                    <p class="mt-1 text-xs text-gray-500">أقصى حجم للملف 50 ميجابايت</p>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                        class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">{{ old('description') }}</textarea>
                    @error('description') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="mb-1 block text-sm font-medium text-gray-700">التصنيف</label>
                    <input type="text" name="category" value="{{ old('category') }}" placeholder="مثال: عقود، فواتير، تقارير..."
                        class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
            ...
            <div class="mt-6 flex items-center gap-3">
                ...
                <a href="{{ route('document-archives.index') }}" class="rounded-lg bg-gray-200 px-6 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-300 transition">إلغاء</a>
            </div>
        </form>
    </div>
</x-app-layout>
