<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-gray-800">تعديل فاتورة مشتريات - {{ $purchaseInvoice->invoice_number }}</h2>
            <a href="{{ route('purchase-invoices.show', $purchaseInvoice) }}" class="inline-flex items-center gap-2 rounded-lg bg-gray-200 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-300 transition">
    ...
    <form action="{{ route('purchase-invoices.update', $purchaseInvoice) }}" method="POST" id="invoiceForm">
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6">
                ...
                <div>
                    ...
                    <select name="currency_id" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        ...
                        @foreach($currencies as $currency)
                            <option value="{{ $currency->id }}" {{ $purchaseInvoice->currency_id == $currency->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                <div>
                    ...
                    <input type="date" name="date" value="{{ old('date', $purchaseInvoice->date?->format('Y-m-d') ?? date('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <input type="date" name="due_date" value="{{ old('due_date', $purchaseInvoice->due_date ?? date('Y-m-d', strtotime('+30 days'))) }}" required
                        class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-gray-700">ملاحظات</label>
                    <input type="text" name="notes" value="{{ old('notes', $purchaseInvoice->notes) }}" placeholder="ملاحظات إضافية..."
                        class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>
            </div>
        ...
        <div class="mt-6 flex items-center gap-3">
            ...
            <a href="{{ route('purchase-invoices.show', $purchaseInvoice) }}" class="rounded-lg bg-gray-200 px-6 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-300 transition">إلغاء</a>
        </div>
    </form>
</x-app-layout>
...
(function() {
    ...
    var existingLines = <?php echo json_encode($purchaseInvoice->lines->map(fn($l) => [
        'item_id' => $l->item_id,
        'quantity' => $l->quantity,
    ...
    function addLine(data) {
        ...
        var inpPrice = makeInput('unit_cost', data.unit_cost || '0', { min: '0' });
    ...
    function calcRow(idx) {
        ...
        var price = parseFloat(tr.querySelector('[name$="[unit_cost]"]').value) || 0;
