<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-gray-800">فواتير المبيعات</h2>
            <a href="{{ route('sales-invoices.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 transition">
    ...
    <div class="rounded-xl bg-white shadow-sm border border-gray-200 p-6">
        <form method="GET" class="mb-6 flex flex-wrap items-end gap-4">
            ...
            <a href="{{ route('sales-invoices.index') }}" class="rounded-lg bg-gray-200 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-300 transition">إعادة تعيين</a>
        </form>
                ...
                <tbody>
                    @forelse($invoices as $invoice)
                        <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                            <td class="px-4 py-3">
                                <a href="{{ route('sales-invoices.show', $invoice) }}" class="font-mono text-xs font-bold text-blue-600 hover:text-blue-800">{{ $invoice->invoice_number }}</a>
                            </td>
                            <td class="px-4 py-3 text-gray-600">{{ $invoice->date->format('Y-m-d') }}</td>
                            <td class="px-4 py-3 text-gray-900">{{ $invoice->customer->name ?? '-' }}</td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="inline-flex items-center gap-1">
                                    <a href="{{ route('sales-invoices.show', $invoice) }}" class="rounded p-1 text-gray-500 hover:bg-blue-50 hover:text-blue-600 transition" title="عرض">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </a>
                                    @if(in_array($invoice->status, ['draft', 'posted']))
                                        <a href="{{ route('sales-invoices.edit', $invoice) }}" class="rounded p-1 text-gray-500 hover:bg-amber-50 hover:text-amber-600 transition" title="تعديل">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                        </a>
                                    @endif
                                    @if($invoice->status === 'draft')
                                        <form action="{{ route('sales-invoices.post', $invoice) }}" method="POST" class="inline" onsubmit="return confirm('هل أنت متأكد من ترحيل هذه الفاتورة؟')">
                                            @csrf
                                        ...
                                        <form action="{{ route('sales-invoices.destroy', $invoice) }}" method="POST" class="inline" onsubmit="return confirm('هل أنت متأكد من حذف هذه الفاتورة؟')">
                                            @csrf
                                            @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-4 py-8 text-center text-gray-500">لا توجد فواتير مبيعات</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
