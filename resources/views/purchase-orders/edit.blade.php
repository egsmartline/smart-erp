<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-gray-800">تعديل أمر شراء - {{ $purchaseOrder->order_number }}</h2>
            <a href="{{ route('purchase-orders.show', $purchaseOrder) }}" class="inline-flex items-center gap-2 rounded-lg bg-gray-200 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-300 transition">
    ...
    <form action="{{ route('purchase-orders.update', $purchaseOrder) }}" method="POST" x-data="orderForm()" x-init="init()">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                <div class="rounded-xl bg-white shadow-sm border border-gray-200 p-6">
                    <h3 class="text-lg font-bold text-gray-800 mb-4">بيانات أمر الشراء</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="mb-1 block text-sm font-medium text-gray-700">المورد <span class="text-red-500">*</span></label>
                            <select name="supplier_id" required class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <input type="date" name="date" value="{{ old('date', $purchaseOrder->date->format('Y-m-d')) }}" required class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        </div>
                        <div>
                            <label class="mb-1 block text-sm font-medium text-gray-700">التاريخ المتوقع</label>
                            <input type="date" name="expected_date" value="{{ old('expected_date', $purchaseOrder->expected_date?->format('Y-m-d')) }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        </div>
                        <div>
                            ...
                            <select name="payment_term_id" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                                ...
                                @foreach($paymentTerms as $term)
                                    <option value="{{ $term->id }}" {{ old('payment_term_id', $purchaseOrder->payment_term_id) == $term->id ? 'selected' : '' }}>{{ $term->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <tbody>
                                <template x-for="(line, index) in lines" :key="index">
                                    <tr class="border-b border-gray-100">
                                        ...
                                        <td class="px-3 py-2 text-left">
                                            <input type="number" x-model.number="line.unit_cost" :name="'lines[' + index + '][unit_cost]'" step="0.01" min="0" required class="w-28 rounded-lg border border-gray-300 px-2 py-1.5 text-sm text-left focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                                        </td>
                                        <td class="px-3 py-2 text-left">
            ...
            <div class="space-y-6">
                ...
                <div class="rounded-xl bg-white shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <textarea name="notes" rows="3" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="ملاحظات داخلية...">{{ old('notes', $purchaseOrder->notes) }}</textarea>
                        </div>
                        <div>
                            <label class="mb-1 block text-sm font-medium text-gray-700">الشروط</label>
                            <textarea name="terms" rows="3" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="شروط الشراء...">{{ old('terms', $purchaseOrder->terms) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('purchase-orders.show', $purchaseOrder) }}" class="rounded-lg bg-gray-200 px-6 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-300 transition">إلغاء</a>
                </div>
            </div>
        </div>
    </form>
        ...
        function orderForm() {
            return {
                ...
                updateItemPrice(index) {
                    ...
                    if (item) {
                        this.lines[index].unit_cost = item.cost_price || 0;
