...
class TransferController extends TenantAwareController
{
    ...
    public function store(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $source = $this->resolveSource($validated['from_type'], $validated['from_id'], $tenantId);
            $target = $this->resolveTarget($validated['to_type'], $validated['to_id'], $tenantId);

            if ($source['balance'] < $validated['amount']) {
                DB::rollBack();
                return back()->withErrors(['error' => 'الرصيد غير كافٍ في الحساب المصدر'])->withInput();
            }

            $refNum = 'TRF-' . date('Ymd') . '-' . strtoupper(substr(uniqid(), -5));

            $this->createSourceTransaction($validated, $tenantId, $userId, $refNum);
            $this->createTargetTransaction($validated, $tenantId, $userId, $refNum);
            $this->updateBalances($validated, $source, $target);
            $this->createJournalEntry($validated, $tenantId, $source, $target, $refNum);
